...
function setupDefaultModalValues() {
	...
	$("#requires-gang").prop("checked", false).change();
...
$("#select-doors-btn").click(function() {
	...
	$.post(`https://${resName}/selectDoors`, JSON.stringify({doorsId: doorsId}), function(data) {
		if(data.doors) {
			setUsedDoors(data.doors);

			if(data.iconCoords) {
				$("#icon-coords-x").val(data.iconCoords.x).change();
				$("#icon-coords-y").val(data.iconCoords.y).change();
				$("#icon-coords-z").val(data.iconCoords.z).change();
			}
		}
